...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 z-40 bg-primary-900/80 backdrop-blur-sm md:hidden"
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-4 border-b border-primary-800">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-primary-500 font-bold text-white">
              MT
            </div>
          ...
          <button 
            ...
            className="hidden md:block text-primary-300 hover:text-white"
          >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
          {filteredNavItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) => cn(
                ...
                isActive
                  ...
                  : "text-primary-200 hover:bg-primary-800 hover:text-white border-l-4 border-transparent"
              )}
            ...
            >
              ...
              {isCollapsed && (
                <div className="absolute left-full ml-4 px-2 py-1 bg-primary-800 text-white text-sm rounded opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50">
                  {item.label}
                </div>
              )}
            </NavLink>
        ...
        <div className="border-t border-primary-800 p-4">
          <div className={cn("flex items-center gap-3", isCollapsed && "justify-center")}>
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary-700 border border-primary-600 text-sm font-bold text-white">
          ...
          <button 
            ...
            className={cn(
              "mt-4 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-red-400 hover:bg-primary-800 hover:text-red-300 transition-colors",
