...
export default function BatchesPage() {
  ...
  const [deleteDialog, setDeleteDialog] = useState<{ isOpen: boolean; batchId: string; batchCode: string }>({
    isOpen: false, batchId: '', batchCode: ''
  });
  ...
  const { register, handleSubmit, reset, setValue, formState: { errors, isSubmitting } } = useForm<BatchForm>({
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: BatchForm }) =>
      batchApi.update(id, { mineralType: data.mineralType, initialWeight: data.initialWeight, extractionDate: data.extractionDate, mineId: data.mineId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['batches'] });
  ...
  const handleEdit = (batch: any) => {
    ...
    setValue('extractionDate', batch.extractionDate?.split('T')[0] || new Date().toISOString().split('T')[0]);
        ...
        <div className="flex items-center gap-3">
          ...
          {user?.role === 'ADMIN' && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                setDeleteDialog({ isOpen: true, batchId: row.id, batchCode: row.batchCode });
              }}
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        onCancel={() => setDeleteDialog({ isOpen: false, batchId: '', batchCode: '' })}
        ...
        <Modal
          ...
          footer={
            <>
              <button
                type="button"
                onClick={() => { setIsModalOpen(false); setEditingBatch(null); }}
