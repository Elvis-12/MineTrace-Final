...
export default function OrganizationsPage() {
  ...
  const [deleteDialog, setDeleteDialog] = useState<{ isOpen: boolean; orgId: string; orgName: string }>({
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDeleteDialog({ isOpen: false, orgId: '', orgName: '' });
    },
    onError: (error: any) => {
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); navigate(ROUTES.USERS); }}
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              setDeleteDialog({ isOpen: true, orgId: row.id, orgName: row.name });
            }}
            className="text-sm font-medium text-red-600 hover:text-red-900 transition-colors flex items-center"
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        onCancel={() => setDeleteDialog({ isOpen: false, orgId: '', orgName: '' })}
        onConfirm={() => deleteMutation.mutate(deleteDialog.orgId)}
        title="Delete Organization"
        message={`Are you sure you want to permanently delete "${deleteDialog.orgName}"? This action cannot be undone.`}
        confirmLabel="Delete"
        danger
