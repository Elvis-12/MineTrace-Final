...
export default function MovementsPage() {
  ...
  const [deleteDialog, setDeleteDialog] = useState<{ isOpen: boolean; movementId: string }>({ isOpen: false, movementId: '' });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: MovementForm }) =>
      movementApi.update(id, {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => movementApi.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['movements'] });
      toast.success('Movement deleted successfully');
      setDeleteDialog({ isOpen: false, movementId: '' });
    },
    onError: (error: any) => {
  ...
  const handleEdit = (movement: any) => {
    ...
    setValue('batchId', movement.batchId);
    setValue('eventType', movement.eventType);
    setValue('fromLocation', movement.fromLocation);
    setValue('toLocation', movement.toLocation);
    setValue('weight', movement.weight);
    setValue('vehicle', movement.vehicle || '');
    setValue('driverName', movement.driverName || '');
    setValue('notes', movement.notes || '');
    setBatchSearch(movement.batchCode);
    setIsModalOpen(true);
  };
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          {user?.role === 'ADMIN' && (
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); setDeleteDialog({ isOpen: true, movementId: row.id }); }}
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        onCancel={() => setDeleteDialog({ isOpen: false, movementId: '' })}
        onConfirm={() => deleteMutation.mutate(deleteDialog.movementId)}
        title="Delete Movement"
        message="Are you sure you want to permanently delete this movement record? This cannot be undone."
        confirmLabel="Delete"
        danger
        loading={deleteMutation.isPending}
      />

      {/* Record / Edit Movement Modal */}
      <Modal
        isOpen={isModalOpen}
        onClose={() => { setIsModalOpen(false); setEditingMovement(null); reset(); setBatchSearch(''); }}
        title={editingMovement ? 'Edit Movement Event' : 'Record Movement Event'}
        size="md"
        footer={
          <>
            <button
              type="button"
              onClick={() => { setIsModalOpen(false); setEditingMovement(null); reset(); setBatchSearch(''); }}
            ...
            <button
              ...
              disabled={isSubmitting || createMutation.isPending || updateMutation.isPending || (!editingMovement && !selectedBatchId)}
            ...
            >
              ...
              {editingMovement ? 'Save Changes' : 'Record Event'}
            </button>
          </>
        }
      >
          ...
          <div>
            ...
            <div className="mt-1 relative">
              <input
                ...
                value={batchSearch}
                onChange={(e) => {
                  if (editingMovement) return; // batch is locked when editing
                  setBatchSearch(e.target.value);
                  setValue('batchId', '');
                }}
                readOnly={!!editingMovement}
                placeholder="Type to search batches..."
                className={`block w-full px-3 py-2 border ${errors.batchId ? 'border-red-300' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm ${editingMovement ? 'bg-gray-50 text-gray-500' : ''}`}
              />
