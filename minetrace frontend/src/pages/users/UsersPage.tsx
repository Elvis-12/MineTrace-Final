...
export default function UsersPage() {
  ...
  const { register: registerEdit, handleSubmit: handleSubmitEdit, reset: resetEdit, setValue: setEditValue, formState: { errors: editErrors, isSubmitting: isEditSubmitting } } = useForm<EditUserForm>({
  ...
  const updateUserMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: EditUserForm }) => authApi.updateUser(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['users'] });
  ...
  const handleEditUser = (user: any) => {
    ...
    setEditValue('organizationId', user.organizationId || '');
  };
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setDeleteDialog({ isOpen: true, userId: row.id, userName: row.fullName });
            }}
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => deleteUserMutation.mutate(deleteDialog.userId)}
        title="Delete User"
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button
              type="button"
              onClick={handleSubmitEdit((data) => updateUserMutation.mutate({ id: editingUser.id, data }))}
              disabled={isEditSubmitting || updateUserMutation.isPending}
              className="inline-flex items-center px-4 py-2 text-sm font-medium text-white border border-transparent rounded-lg bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50"
            >
              {(isEditSubmitting || updateUserMutation.isPending) && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
              Save Changes
            </button>
          </>
        }
      >
        <form className="space-y-4" onSubmit={handleSubmitEdit((data) => updateUserMutation.mutate({ id: editingUser?.id, data }))}>
          <div>
            <label className="block text-sm font-medium text-gray-700">Full Name</label>
            <input
              type="text"
              {...registerEdit('fullName')}
              className={`mt-1 block w-full px-3 py-2 border ${editErrors.fullName ? 'border-red-300' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm`}
            />
            {editErrors.fullName && <p className="mt-1 text-sm text-red-600">{editErrors.fullName.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Email</label>
            <input
              type="email"
              {...registerEdit('email')}
              className={`mt-1 block w-full px-3 py-2 border ${editErrors.email ? 'border-red-300' : 'border-gray-300'} rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm`}
            />
            {editErrors.email && <p className="mt-1 text-sm text-red-600">{editErrors.email.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Role</label>
            <select
